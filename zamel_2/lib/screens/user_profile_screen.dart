    ...
    return Scaffold(
      ...
      body: StreamBuilder<AppUser?>(
        ...
        builder: (context, snapshot) {
          ...
          // وضعنا حماية إضافية هنا لتجنب أخطاء القيم الفارغة
          final canFollow = currentUser != null && currentUser.id != user.id;
          // حماية التأكد من وجود لستة المتابعة قبل فحصها
          final isFollowing = currentUser != null && (currentUser.following?.contains(user.id) ?? false);

          // الحل الأساسي للشاشة الرمادية: إضافة SingleChildScrollView للسماح بالتمرير
              ...
              children: [
                CircleAvatar(
                  ...
                  child: (user.photoURL == null || user.photoURL!.isEmpty) 
                      ? Text(user.username.isNotEmpty ? user.username[0].toUpperCase() : 'م', style: const TextStyle(fontSize: 32)) 
                      : null,
                ),
                ...
                Row(
                  ...
                  children: [
                    // حماية الأرقام في حالة عدم وجود متابعين لسه
                    _buildStat('متابع', user.followers?.length ?? 0),
                    _buildStat('يتابع', user.following?.length ?? 0),
                    _buildStat('نقاط', user.points ?? 0),
                  ],
                ),
                const SizedBox(height: 20),
