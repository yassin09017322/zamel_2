...
class FeedProvider extends ChangeNotifier {
  ...
  Stream<List<Post>> postsStream({String? categoryId}) {
    Query<Map<String, dynamic>> query = firestore.collection('posts').orderBy('timestamp', descending: true);

    if (categoryId != null && categoryId.isNotEmpty && categoryId != 'all') {
      query = query.where('categoryId', isEqualTo: categoryId).orderBy('timestamp', descending: true);
    }
