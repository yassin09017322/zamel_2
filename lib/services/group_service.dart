...
class GroupService {
  ...
  final Map<String, Future<void>> _groupSettingsWriteLocks =
      <String, Future<void>>{};
  ...
  static bool _isGroupModerator(Group group, String userId) {
    ...
    return safeUserId.isNotEmpty &&
        group.moderators.any((id) => id.trim() == safeUserId);
  }

  static bool _isGroupMember(Group group, String userId) {
    final safeUserId = userId.trim();
    return safeUserId.isNotEmpty &&
        group.memberIds.any((id) => id.trim() == safeUserId);
  }
  ...
  Stream<Group?> groupStream(String groupId, {String? userId}) {
    return _groups.doc(groupId).snapshots().map((snapshot) {
      ...
      if (group.isPrivate && requesterId.isNotEmpty) {
        if (_isGroupOwner(group.ownerId, requesterId) ||
            _isGroupMember(group, requesterId)) {
          return group;
        }
        return null;
      }
  ...
  Future<bool> canManageGroupSettings(
    String groupId,
    String userId, {
    bool includePrivateSetting = false,
  }) async {
    final snapshot = await _groups.doc(groupId).get();
    if (!snapshot.exists) return false;
    final data = snapshot.data() ?? <String, dynamic>{};
    ...
    await _firestore
        .runTransaction((transaction) async {
          ...
          if (currentMembers.contains(normalizedUserId)) {
            transaction.set(
              groupRef.collection('members').doc(normalizedUserId),
              {
          ...
          if (data['isPrivate'] == true) {
            transaction.set(
              groupRef.collection('joinRequests').doc(normalizedUserId),
              {
                'userId': normalizedUserId,
                'status': 'pending',
                'createdAt': FieldValue.serverTimestamp(),
  ...
  Future<void> promoteMemberToAdmin({
    ...
    await _firestore
        .runTransaction((transaction) async {
          ...
          if (targetId == ownerId)
            throw Exception('لا يمكن ترقية مالك المجموعة');
          ...
          if (moderators.contains(targetId)) {
            transaction.set(
              groupRef.collection('members').doc(targetId),
              {
                'userId': targetId,
                'role': 'admin',
                'joinedAt': FieldValue.serverTimestamp(),
  ...
  Future<void> demoteAdmin({
    ...
    await _firestore
        .runTransaction((transaction) async {
          ...
          if (targetId.isEmpty) throw Exception('معرف المستخدم غير صالح');
          if (targetId == ownerId)
            throw Exception('لا يمكن إلغاء صلاحيات مالك المجموعة');
    ...
    await _firestore
        .runTransaction((transaction) async {
          ...
          if (targetUserId == ownerId)
            throw Exception('لا يمكن إضافة مالك المجموعة كعضو');
          await _requireExistingUser(targetUserId);

          final members = _normalizeUserList(data['memberIds']);
          if (members.contains(targetUserId)) {
            transaction.set(
              groupRef.collection('members').doc(targetUserId),
              {
                'userId': targetUserId,
                'role': 'member',
                'joinedAt': FieldValue.serverTimestamp(),
  ...
  Future<void> removeMember({
    ...
    await _firestore
        .runTransaction((transaction) async {
          ...
          if (targetUserId.isEmpty) throw Exception('معرف المستخدم غير صالح');
          if (targetUserId == ownerId)
            throw Exception('لا يمكن إزالة مالك المجموعة');
    ...
    await _firestore
        .runTransaction((transaction) async {
          ...
          if (data['ownerId'] == user.uid)
            throw Exception('لا يمكن لمالك المجموعة مغادرتها');
      ...
      await _firestore
          .runTransaction((transaction) async {
            ...
            if (isPrivate != null) {
              if (!isOwner) {
                throw Exception(
                  'مالك المجموعة فقط يمكنه تعديل خصوصية المجموعة',
                );
              }
  ...
  Future<String> createPost({
    ...
    await postRef
        .set({
          ...
          'mediaType': normalizedMediaType.isEmpty
              ? 'none'
              : normalizedMediaType,
          'likes': <String>[],
          'commentsCount': 0,
  ...
  Future<String> sendGroupMediaMessage({
    ...
    final actorId = senderId.trim().isNotEmpty
        ? senderId.trim()
        : _currentUser.uid;
    final safeGroupId = groupId.trim();
    if (safeGroupId.isEmpty) throw Exception('معرف المجموعة مطلوب');
    ...
    try {
      await NotificationService().createGroupMessageEvent(
        groupId: safeGroupId,
        messageId: messageRef.id,
        actorUserId: actorId,
        actorName: senderName,
        body: normalizedText.isEmpty ? 'أرسل مرفقاً' : normalizedText,
      );
    } catch (_) {
      // Group message delivery must not block the message write.
    }
  ...
  Future<MediaUploadResult> uploadGroupMedia({
    ...
    if (bytes != null && bytes.isEmpty) {
      return const MediaUploadResult(success: false, error: 'ملف الوسائط فارغ');
    }
  ...
  Future<void> acceptJoinRequest({
    ...
    await _firestore
        .runTransaction((transaction) async {
          ...
          transaction.delete(
            groupRef.collection('joinRequests').doc(safeUserId),
          );
          transaction.update(groupRef, {
  ...
  Future<void> _requireMember(String groupId, String userId) async {
    ...
    final membershipDoc = await _groups
        .doc(groupId)
        .collection('members')
        .doc(userId.trim())
        .get();
