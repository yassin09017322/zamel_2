...
class AudioCommentService {
  ...
  Future<String?> startRecording() async {
    ...
    if (defaultTargetPlatform == TargetPlatform.android || defaultTargetPlatform == TargetPlatform.iOS) {
      ...
      await _mobileRecorder.start(const RecordConfig(encoder: AudioEncoder.aacLc), path: filePath);
    ...
    final tempDir = await getTemporaryDirectory();
    final filePath = '${tempDir.path}/comment_${DateTime.now().millisecondsSinceEpoch}.wav';
    await _desktopRecorder.openRecorder();
    await _desktopRecorder.startRecorder(toFile: filePath, codec: Codec.pcm16WAV);
    _recordingPath = filePath;
    _durationSeconds = 0;
    _isRecording = true;
    _timer?.cancel();
  ...
  Future<String?> stopRecording() async {
    ...
    await _desktopRecorder.stopRecorder();
    _isRecording = false;
    return _recordingPath;
  }
  ...
  Future<void> play(String url) async {
    ...
    await _desktopPlayer.openPlayer();
    await _desktopPlayer.startPlayer(fromURI: url);
  }
  ...
  Future<void> pause() async {
    ...
    await _desktopPlayer.pausePlayer();
  }
  ...
  Future<void> stop() async {
    ...
    await _desktopPlayer.stopPlayer();
  }
  ...
  Future<void> dispose() async {
    ...
    await _desktopPlayer.stopPlayer();
    await _desktopPlayer.closePlayer();
    await _desktopRecorder.closeRecorder();
  }
}